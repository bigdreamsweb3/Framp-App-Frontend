...
export function WalletView({
  ...
  useEffect(() => {
    ...
    const fetchWallets = async () => {
      try {
        ...
        const data: WalletMethod = await response.json(); // parse response
        setWallets((prev) => [...prev, data]);// Use response data from backend
      } catch (err) {
  ...
  const handleAddWallet = async () => {
    if (!newWallet.walletAddress || !newWallet.walletName) return;
    const newMethod: WalletMethod = {
      id: `wallet_${Date.now()}`, // Ideally, ID is generated by the backend
      type: "wallet",
      name: newWallet.walletName,
      details: `${newWallet.walletAddress.slice(0, 4)}...${newWallet.walletAddress.slice(-4)}`,
      accountName: newWallet.walletName,
  ...
  const handleSetDefault = async (id: string) => {
    try {
      await fetch(`/api/wallets/${id}/default`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
